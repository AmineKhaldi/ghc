...
--
pprDebugCLabel :: DynFlags -> CLabel -> SDoc
pprDebugCLabel dflags lbl = pprCLabel dflags lbl <> parens extra
   where
      extra = case lbl of
        IdLabel _ _ info
...
pprCLbl (IPE_Label (InfoProvEnt l _ _ (m, _, _))) = pprCode CStyle (ppr l) <> text "_" <> ppr m <> text "_ipe"
